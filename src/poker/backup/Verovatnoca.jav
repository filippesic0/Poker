...
/*			}
		...
		for(k=8;k>=0;k--)
			...
			{	
				...
				boolean ind=true;
				filewriter[0] = new FileWriter("Parovi karata poraz.txt");
				filewriter[1] = new FileWriter("Parovi karata remi.txt");
				filewriter[2] = new FileWriter("Parovi karata pobeda.txt");
				for(j=0;j<3;j++)
					writer[j] = new PrintWriter(filewriter[j]);
				rezultat1=new int[3];
				for(j=0;j<l-1;j++)
				{	if(spil.get(j)==par[0]||spil.get(j)==par[1])
						continue;
					karte7[5]=brojiboja(spil.get(j));
					for(i=j+1;i<l;i++)
					{	if(spil.get(i)==par[0]||spil.get(i)==par[1])
							continue;
						karte7[6]=brojiboja(spil.get(i));
						ruka5=ruka(karte7);
						int rez=Ruka.poredjenje(ruka7,ruka5);
						rezultat1[rez+1]++;
						int spili=spil.get(i);
						int spilj=spil.get(j);
						//if(parovi[spili][spilj]!=1-rez||parovi[spilj][spili]!=1-rez)
						{	if(karte7[5].id>=karte7[6].id)
								writer[rez+1].println(karte7[5].toString()+karte7[6].toString());
							else
								writer[rez+1].println(karte7[6].toString()+karte7[5].toString());
							ind=true;
						}
					}
				}
				...
				if(ind&&(rezultat[0]!=rezultat1[0]||rezultat[1]!=rezultat1[1]))
					I--;
		//	*/	
			}
/*		Main.time();
		...
		int t,r,pobeda=0,remi=0,poraz=0;
		karte5=new Karta[7];
		ruka5=new Karta[7];
		//ArrayList<Integer> 
        spil=new ArrayList<>();
		...
		l=spil.size();
		for(t=0;t<l-1;t++)
		{	karte7[5]=brojiboja(spil.get(t));
			karte5[3]=brojiboja(spil.get(t));
			for(r=t+1;r<l;r++)
			{	if(r==t)
					continue;
				karte7[6]=brojiboja(spil.get(r));
				karte5[4]=brojiboja(spil.get(r));
				ruka7=ruka(karte7);
				for(i=0;i<l-1;i++)
				{	if(i==t||i==r)
						continue;
					karte5[5]=brojiboja(spil.get(i));
					for(j=i+1;j<l;j++)
					{	if(j==t||j==r)
							continue;
						karte5[6]=brojiboja(spil.get(j));
						ruka5=ruka(karte5);
					}
				}
				int rez=Ruka.poredjenje(ruka7,ruka5);
        ...
        Main.time();
		double verovatnoca2=(pobeda+(float)remi/2)/(pobeda+remi+poraz);
	*/	
